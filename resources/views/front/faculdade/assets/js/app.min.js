                ...
                function A(t) {
                    if (!w.prop("readonly")) {
                        var n, a, i, o = t.which || t.keyCode,
                            l = w.caret(),
                            f;
                        t.ctrlKey || t.altKey || t.metaKey || o < 32 || !o || 13 === o || (l.end - l.begin != 0 && (R(l.begin, l.end), b(l.begin, l.end - 1)), (n = p(l.begin - 1)) < h && (a = String.fromCharCode(o), u[n].test(a)) && (k(n), E[n] = a, S(), i = p(n), r ? setTimeout((function() { e.proxy(e.fn.caret, w, i)() }), 0) : w.caret(i), l.begin <= g && c()), t.preventDefault())
                    }
                }
...
function addCart(id) {
    var cart = [];
    id ? (cart.push({ id: id, parent_id: null, product_type: "concurso", qty: 1 }), addToCart(cart)) : $.edbox({ target: ".modal-options-error" })
}
...
function openVideo(e) {
    var $this = $(e),
        _videoId = $this.attr("data-video-id");
    $this.parents(".subjects-list").find(".cel-video").remove().end().end().parents(".wrapper").append('<div class="cel cel-video"><iframe keep-ratio src="https://www.youtube.com/embed/' + _videoId + '?autoplay=1&modestbranding=1&rel=0&color=white" frameborder="0" allowfullscreen></iframe></div>'), keepRatio()
}

function ajaxFail() { $.edbox({ danger: "Desculpe, houve um erro inesperado! Tente novamente mais tarde." }) }

function addToCart(data) {
    $.ajax({ type: "POST", datatype: "json", url: "https://carrinho.aprovaconcursos.com.br/add-item", data: { items: data } }).fail((function(xhr, status, errorThrown) { console.log(xhr), console.error(status), console.error(errorThrown), ajaxFail() })).done((function(data) {
        if (/^(\{|\[])/g.test(data)) {
...
$(document).ready((function() {
    var e = $(".link-inteiro").width(),
        ...
        o = $(".course-name:not(.course-enem)").outerWidth(),
        a = $(".link-inteiro-pos ").find(".course-name").outerWidth(),
        n = $(".detail-duracao").outerWidth(),
        c = $(".detail-investimento:not(.course-enem)").outerWidth(),
    ...
    $(".cabecalho-ext").find(".name").css("min-width", d), $(window).on("resize", (function() {
        ...
        $(".cabecalho-ext").find(".name").css("min-width", d), $(".cabecalho").width(e), $(".cabecalho-pos").width(i), $(".cabecalho-ext").width(t), $(".name-pos").css("min-width", a), $(".name").css("min-width", o), $(".duracao").css("min-width", n), $(".investimento").css("min-width", c), $(".carga").css("min-width", r), $(".inicio").css("min-width", s), keepRatio()
    }));
    var wLocation = window.location.pathname,
        h = $(".showcase").outerHeight(),
        l, u = $("#graduacao").outerHeight() + $("#posGraduacao").height();
    $(window).on("scroll", (function() { valor_atual = $(document).scrollTop(), "/" == wLocation || "/ead/" == wLocation ? valor_atual >= $("#graduacao").offset().top ? ($(".cabecalho").addClass("fixa"), valor_atual >= $("#educacao").offset().top ? ($(".cabecalho").hide(), $(".cabecalho-pos").addClass("fixa"), $(".cabecalho-pos").show(), valor_atual >= $(".section-top-courses").offset().top && ($(".cabecalho-pos").hide(), $(".cabecalho-pos").removeClass("fixa"))) : ($(".cabecalho").show(), $(".cabecalho-pos").hide(), $(".cabecalho-pos").removeClass("fixa"))) : $(".cabecalho").removeClass("fixa") : valor_atual >= headerHeight ? ($(".cabecalho").addClass("fixa"), $(".cabecalho-pos").addClass("fixa")) : ($(".cabecalho").removeClass("fixa"), $(".cabecalho-pos").removeClass("fixa")) })), $(".vermais").on("click", (function() {
    ...
    var id;
    e.preventDefault(), addCart($(this).attr("aula-id"))
